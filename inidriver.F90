    ...
    !Sources
    character(LEN=Ini_max_string_len) S,TransferClFileNames(max_transfer_redshifts),Zfilename
    ...
    integer :: Nz
    integer, parameter :: fio_unit = 193 !Any number not used elsewhere will do
    real(dl) :: res
    !real(dl), allocatable :: pvals, p2
    real(dl), dimension(100000) :: zvals
    real(dl), dimension(100000) :: pvals
    !real(dl), allocatable :: p2(:)
    logical fexists
    ...
    do i=1, num_redshiftwindows
        ...
        if (RedWin%kind /= window_21cm) then
            ...
            Zfilename = Ini_Read_String('redshift_file('//trim(numstr)//')')
            
            inquire(FILE=Zfilename,EXIST=fexists)

            if (fexists) then
               ! Read from file
               Nz = 1

               write (*,*) 'Found specified redshift distribution file. Using it.'

               call OpenTxtFile(Zfilename,fio_unit)

               do
                  read(fio_unit,*, end=500) zvals(Nz),pvals(Nz)
                  !write (*,*) zvals(Nz),pvals(Nz)
                  Nz = Nz + 1
...
500            close(fio_unit)
               ...
               allocate(RedWin%zvals(1:Nz))
               allocate(RedWin%pvals(1:Nz))
               allocate(RedWin%p2(1:Nz))

               RedWin%zvals = zvals(1:Nz)
               RedWin%pvals = pvals(1:Nz)

               
               call spline_double(RedWin%zvals, RedWin%pvals, Nz, Redwin%p2)

               RedWin%Nz = Nz
               RedWin%minz = minval(RedWin%zvals)
               RedWin%maxz = maxval(RedWin%zvals)

               RedWin%fromFile = .true.
               !write (*,*) size(RedWin%zvals),size(RedWin%pvals),size(RedWin%p2),Nz
               !write (*,*) RedWin%maxz
               
               !call OpenTxtFile(Zfilename,fio_unit)
               !Nz = 0
               !do
               !   read(fio_unit,*, end=600) zvals(Nz),pvals(Nz)
               !   call spline_eval(zvals(Nz), RedWin%zvals, RedWin%pvals, RedWin%p2, RedWin%Nz,res)
               !   write (*,*) zvals(Nz),pvals(Nz),res
               !   Nz = Nz + 1
               !end do
...
!600            close(fio_unit)
               ...
               write (*,*) 'No redshift distribution file found or specified. Using mean and sigma.'
               RedWin%fromFile = .false.
            end if
